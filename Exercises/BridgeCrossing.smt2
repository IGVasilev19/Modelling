...
(assert
  (forall ((crossing Int))
    (=> (and (<= 1 crossing) (<= crossing 5));In the best possible scenario all of them should have crossed in a total of 5 crossings
      (ite (= (mod crossing 2) 0);We check if the number of the current crossing is even or odd. If it is odd that mean that the 'torch' is on the initial side of the bridge
                                 ;so two people should cross and if it is even then a person should be sent back with the torch so another pair can cross.
        (and
            ;Selects the person who will return the torch, for each possible person to be chosen it first checks if that person is already on the opposite site of the bridge.
            (or 
            ...
            (= (timeStepSum crossing) (people (crosser1 crossing)));The time it took for each crossing is saved in a declare-fun timeStepSum, which maps the crossing to the time it took to complete it
            (= (personCrossed (+ crossing 1) 1) (ite (= (crosser1 crossing) 1) false (personCrossed crossing 1)));For each person we either set the personCrossed to false is that was the chosen person
            (= (personCrossed (+ crossing 1) 2) (ite (= (crosser1 crossing) 2) false (personCrossed crossing 2)));to return, or we persist their current personCrossed state for the next crossing.
        ...
        (and
            (or 
                ;It starts selecting the first person for the pair which will cross. First checks if the person is currently on the initial side of the bridge, then if chosen, sets
                ;the personCrossed state for the next crossing to true.
            ...
            (or 
                (and (= (personCrossed crossing 1) false) (= (crosser2 crossing) 1) (= (personCrossed (+ crossing 1) 1) true));Equivalet for the second person of the pair. 
            ...
            (not (= (crosser1 crossing) (crosser2 crossing)));Ensures that the two chosen people are not the same person
            (ite (> (people (crosser1 crossing)) (people (crosser2 crossing)));Checks which person has a slower pace and that is set as the cost of the crossing. 
                    (= (timeStepSum crossing) (people (crosser1 crossing))) 
                    (= (timeStepSum crossing) (people (crosser2 crossing)))
            )
            (= (personCrossed (+ crossing 1) 1) (ite (or (= (crosser1 crossing) 1) (= (crosser2 crossing) 1)) true (personCrossed crossing 1)));Switches the personCrossed state of the two chosen people to
            (= (personCrossed (+ crossing 1) 2) (ite (or (= (crosser1 crossing) 2) (= (crosser2 crossing) 2)) true (personCrossed crossing 2)));true for the next crossing and persists the states of the rest.
...
(assert (= sum (+ (timeStepSum 1) (timeStepSum 2) (timeStepSum 3) (timeStepSum 4) (timeStepSum 5))));Add up the costs of all recorded crossings together to get their sum.
(minimize sum);Set the solver to find a solution which minimizes the total sum.
